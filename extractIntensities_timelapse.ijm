...
for (i=0;i<cells.length;i++) {
	...
	for (j=0;j<tps.length;j++) {
		...
		if (tps[j]=="kin_afterCID_") {				
			...
			mitos = Table.getString(spacer+"kin_afterCID_"+"mito_roi.zip",i,"paths");
...
for (i=0;i<cells.length;i++) {
	...
	saveP = dataPath+"processed/results/"+cells[i]+"_intensities.csv";
	...
	Table.set("FrameTimeRel", 0, t0Start-kinStart+4);
	Table.set("FrameTimeAbs", 1, tendStart);
	Table.set("FrameTimeRel", 1, tendStart-kinStart+4);
	j=0;
	while (j<frameStarts.length) {
		Table.set("FrameTimeRel", (j/2)+2, frameStarts[j]+4);
		Table.set("FrameTimeAbs", (j/2)+2, (kinStart+frameStarts[j])+4);
		j=j+2;
	}

	// Results table format
	if(!File.isDirectory(dataPath+"processed/results")) {
		File.makeDirectory(dataPath+"processed/results");
	}
